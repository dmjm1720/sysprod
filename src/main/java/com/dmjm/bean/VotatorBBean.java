...
public class VotatorBBean implements Serializable {
	...
	public void guardarVotator() {
		...
		if (validacion) {
			...
			for (Date fec : listarFechas) {
				...
				folio = folDao.buscarFolioVotatorB(newYear);
				...
				folioDao.actualizarFolioVotatorB(newYear, folio);
			}
			String script = "setTimeout(function() { window.location.href='VotatorB.html'; }, 3000);";
			PrimeFaces.current().executeScript(script);
		}
	...
	public void actualizarVotator() {
		...
		actualizarPromedios(votatorEditar.getFolioPreparacionVotatorB().getIdFolioPrep());
	...
	public void visualizarReporteExcel() throws SQLException {
		...
		FacesContext facesContext = FacesContext.getCurrentInstance();
		ServletContext servletContext = (ServletContext) facesContext.getExternalContext().getContext();
		String ruta = servletContext.getRealPath("/REP/votator_rep_b_excel.jasper");
