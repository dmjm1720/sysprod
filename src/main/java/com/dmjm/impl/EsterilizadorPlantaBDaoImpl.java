...
public class EsterilizadorPlantaBDaoImpl extends Conexion implements IEsterilizdorPlantaBDao {
	...
	public void actualizarPromedioFlujoLitros(int folio) {
		try {
			...
			String sql = "UPDATE ESTERILIZADOR_PLANTA_B  SET FLUJO_LITROS_HORA = (SELECT COALESCE(AVG(FLUJO_LITROS_HORA), 0) FROM ESTERILIZADOR_PLANTA_B WHERE ID_FOLIO_PREP=? AND HORA NOT IN ('PROM.')) WHERE HORA='PROM.' AND ID_FOLIO_PREP=?";
			PreparedStatement ps = getCnSysProd().prepareStatement(sql);
