...
public class CocedoresDaoImpl extends Conexion implements ICocedoresDao {
	...
	public void actualizarPromedioCondensacion(int folio) {
		try {
			...
			String sql = "UPDATE COCEDORES SET CONDENSACION = (SELECT COALESCE(AVG(CONDENSACION), 0) FROM COCEDORES WHERE ID_FOLIO_PREP=? AND HORA_LIMITES_ESPECIFICOS NOT IN ('PROM.')) WHERE HORA_LIMITES_ESPECIFICOS='PROM.' AND ID_FOLIO_PREP=?;";
			PreparedStatement ps = getCnSysProd().prepareStatement(sql);
