...
public class LimpiezaVotatorADaoImpl extends Conexion implements ILimpiezaVotatorADao {
	private static final Logger LOGGER = LogManager.getLogger(LimpiezaVotatorADaoImpl.class.getName());

	@Override
	public List<LimpiezaVotatorA> listarLimpieza(int folio) {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			String hql = "FROM LimpiezaVotatorA c JOIN FETCH c.folioPreparacionVotatorA WHERE  c.folioPreparacionVotatorA.idFolioPrep = :folio";
			Query<LimpiezaVotatorA> query = session.createQuery(hql, LimpiezaVotatorA.class);
	...
	@Override
	public void guardarLimpieza(LimpiezaVotatorA limpieza) {
	...
	@Override
	public void actualizarLimpieza(LimpiezaVotatorA limpieza) {
		Session session = null;
		try {
	...
	public int validarNoLimpieza(int folioPrep) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery("SELECT COALESCE(MAX(NO_LIMPIEZA), 0) + 1 "
					+ "FROM LIMPIEZA_VOTATOR_A " + "WHERE ID_FOLIO_PREP = :idFolioPrep");
	...
	@Override
	public List<VotatorA> listarTodo() {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			String hql = "FROM LimpiezaVotatorA WHERE voBo='PENDIENTE'";
	...
	public void actualizarTodoLimpieza() {
		...
		try {
			...
			Query<?> query = session
					.createSQLQuery("UPDATE LIMPIEZA_VOTATOR_A SET VOBO = 'APROBADO' WHERE VOBO='PENDIENTE'");
			query.executeUpdate();
	...
	public List<Integer> noLimpieza(int folio) throws SQLException {
		...
		PreparedStatement ps = getCnSysProd()
				.prepareStatement("SELECT DISTINCT (NO_LIMPIEZA) FROM LIMPIEZA_VOTATOR_A WHERE ID_FOLIO_PREP = ?");
		ps.setLong(1, folio);
	...
	public void borrarLimpieza(int folio, int noLimpieza) {
		...
		try {
			...
			Query<?> query = session.createQuery(
					"DELETE FROM LimpiezaVotatorA WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
			query.setParameter("folio", folio);
	...
	public void agregarVoBo(int folio, int noLimpieza, int idUsuario) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery(
					"UPDATE LIMPIEZA_VOTATOR_A SET VOBO = 'APROBADO', ID_USUARIO = :idUsuario WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
	...
	public void borrarVoBo(int folio, int noLimpieza) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery(
					"UPDATE LIMPIEZA_VOTATOR_A SET VOBO = 'PENDIENTE', ID_USUARIO=1028 WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
			query.setParameter("folio", folio);
			query.setParameter("noLimpieza", noLimpieza);
			query.executeUpdate();
