...
public class UltraFiltracionDosDaoImpl extends Conexion implements IUltraFiltracionDosDao {
	...
	public void actualizarManto(int folio) {
		try {
			...
			String sql = "UPDATE ULTRAFILTRACION_DOS SET ESTADO_MANTO=1 WHERE HORA='PROM.' AND ID_FOLIO_PREP=?;";
			PreparedStatement ps = getCnSysProd().prepareStatement(sql);
	...
	public void actualizarLimpieza(int folio, int estado) {
		try {
			...
			String sql = "UPDATE ULTRAFILTRACION_DOS SET ESTADO_LIMPIEZA=? WHERE HORA='PROM.' AND ID_FOLIO_PREP=?;";
			PreparedStatement ps = getCnSysProd().prepareStatement(sql);
			ps.setInt(1, estado);
			ps.setInt(2, folio);
