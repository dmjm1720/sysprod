...
public class LimpiezaDaoImpl extends Conexion implements ILimpiezaDao {
	...
	public int validarNoLimpieza(int folioPrep) {
		...
		try {
			session = HibernateUtil.getSessionFactory().openSession();
			Query<?> query = session.createSQLQuery("SELECT COALESCE(MAX(NO_LIMPIEZA), 0) + 1 " + "FROM LIMPIEZA "
	...
	public void actualizarTodoLimpieza() {
		...
		try {
			...
			Query<?> query = session.createSQLQuery("UPDATE LIMPIEZA SET VOBO = 'APROBADO' WHERE VOBO='PENDIENTE'");
			query.executeUpdate();
	...
	public List<Integer> noLimpieza(int folio) throws SQLException {
		...
		PreparedStatement ps = getCnSysProd()
				.prepareStatement("SELECT DISTINCT (NO_LIMPIEZA) FROM LIMPIEZA WHERE ID_FOLIO_PREP = ?");
		ps.setLong(1, folio);
