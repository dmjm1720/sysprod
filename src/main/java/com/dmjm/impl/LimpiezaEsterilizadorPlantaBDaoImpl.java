...
public class LimpiezaEsterilizadorPlantaBDaoImpl extends Conexion implements ILimpiezaEsterilizadorPlantaBDao {
	
	private static final Logger LOGGER = LogManager.getLogger(LimpiezaEsterilizadorPlantaBDaoImpl.class.getName());
	...
	public void actualizarLimpieza(LimpiezaEstB limpieza) {
		...
		try {
			...
			transaction.commit();
			String info = "Se ha actualizado el registro limpieza";
	...
	public List<Integer> noLimpieza(int folio) throws SQLException {
		...
		PreparedStatement ps = getCnSysProd()
				.prepareStatement("SELECT DISTINCT (NO_LIMPIEZA) FROM LIMPIEZA_EST_B WHERE ID_FOLIO_PREP = ?");
		ps.setLong(1, folio);
	...
	public void borrarLimpieza(int folio, int noLimpieza) {
		...
		try {
			...
			Query<?> query = session.createQuery("DELETE FROM LimpiezaEstB WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
			query.setParameter("folio", folio);
	...
	public void agregarVoBo(int folio, int noLimpieza, int idUsuario) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery(
					"UPDATE LIMPIEZA_EST_B SET VOBO = 'APROBADO', ID_USUARIO = :idUsuario WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
	...
	public void borrarVoBo(int folio, int noLimpieza) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery(
					"UPDATE LIMPIEZA_EST_B SET VOBO = 'PENDIENTE', ID_USUARIO=1028 WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
			query.setParameter("folio", folio);
			query.setParameter("noLimpieza", noLimpieza);
			query.executeUpdate();
