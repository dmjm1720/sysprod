...
public class LimpiezaUltraDosDaoImpl extends Conexion implements ILimpiezaUltraDosDao {
	private static final Logger LOGGER = LogManager.getLogger(LimpiezaUltraDosDaoImpl.class.getName());
	...
	public List<Integer> noLimpieza(int folio) throws SQLException {
		...
		PreparedStatement ps = getCnSysProd()
				.prepareStatement("SELECT DISTINCT (NO_LIMPIEZA) FROM LIMPIEZA_ULTRA_DOS WHERE ID_FOLIO_PREP = ?");
		ps.setLong(1, folio);
	...
	public void borrarLimpieza(int folio, int noLimpieza) {
		...
		try {
			...
			Query<?> query = session.createQuery(
					"DELETE FROM LimpiezaUltraDos WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
			query.setParameter("folio", folio);
	...
	public void agregarVoBo(int folio, int noLimpieza, int idUsuario) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery(
					"UPDATE LIMPIEZA_ULTRA_DOS SET VOBO = 'APROBADO', ID_USUARIO = :idUsuario WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
	...
	public void borrarVoBo(int folio, int noLimpieza) {
		...
		try {
			...
			Query<?> query = session.createSQLQuery(
					"UPDATE LIMPIEZA_ULTRA_DOS SET VOBO = 'PENDIENTE', ID_USUARIO=1028 WHERE ID_FOLIO_PREP = :folio AND NO_LIMPIEZA = :noLimpieza");
			query.setParameter("folio", folio);
			query.setParameter("noLimpieza", noLimpieza);
			query.executeUpdate();
