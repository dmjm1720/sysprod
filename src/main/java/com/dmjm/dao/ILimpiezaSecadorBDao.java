...
public interface ILimpiezaSecadorBDao {
	List<LimpiezaSecadorB> listarLimpieza(int folio);

	void guardarLimpieza(LimpiezaSecadorB limpieza);

	void actualizarLimpieza(LimpiezaSecadorB limpieza);

	int validarNoLimpieza(int folioPrep);

	List<LimpiezaSecadorB> listarTodo();
