...
public interface ILimpiezaSecadorADao {
	List<LimpiezaSecadorA> listarLimpieza(int folio);

	void guardarLimpieza(LimpiezaSecadorA limpieza);

	void actualizarLimpieza(LimpiezaSecadorA limpieza);

	int validarNoLimpieza(int folioPrep);

	List<LimpiezaSecadorA> listarTodo();
